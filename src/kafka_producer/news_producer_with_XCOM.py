...
# - 최종적으로 "모든 메시지 전송 완료", "Producer 종료" 및 "Total messages produced: [개수]" 메시지가 출력됨
#
# 결과 확인 방법:
...
def main():
    message_count = 0  # 메시지 개수 카운터 초기화
    try:
        ...
        for source, category, rss_url in RSS_FEED_URLS:
            ...
            for entry in feed.entries:
                ...
                message_count += 1  # 메시지 전송 시 카운트 증가
                print(f"✅[kafka] [{time.strftime('%Y-%m-%d %H:%M:%S')}] 전송 완료 ({message_count}번째): {title}")

        # 모든 메시지가 전송될 때까지 대기
        producer.flush()
        print("모든 메시지 전송 완료")
        # 최종 메시지 개수를 표준 출력으로 내보내 XCom으로 캡처되도록 함
        print(f"Total messages produced: {message_count}", flush=True)
